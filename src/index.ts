...
export class Game extends phaser.Game {
  ...
  public offerContract(
    ...
    this.hud = this.hud.setQuestion(
      'Do you want to hire ' + character.name + ' for $' + (Math.round(character.getSalary()*100)/100) + '?' + '\n' +
      'Str=' + character.strength + ', Speed=' +Math.round(character.speed/100)+ ', Int='+character.intelligence,
      ['yes', 'no'],
      (option: number) => {
        this.isOfferingContract = false;
        this.hud = this.hud.clearQuestion();
        if (option === 1) {
          if (this.worldState.playerCharacter.getMoney() >= character.getSalary()) {
            ...
            this.hud = this.hud.setMessage('You can\'t afford ' + character.name + '...');
          }
