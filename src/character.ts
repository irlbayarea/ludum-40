...
class Character {
    ...
    constructor(name: string, x: number, y: number, speed: number, str: number = 1, int: number = 1, cha: number = 1, isGuard: boolean = false) {
    ...
    public handleCrisis(crisis: Crisis): boolean {
        ...
        if (this.isGuard) {
            return crisis.getOptions()[Math.floor(Math.random()*crisis.getOptions().length)].choose(this);
        }
        else {
            return false;
        }
...
function generateName(): string {
    ...
    const names: string[] = ["Bob", "Joe", "Bill", "Steve", "Eric", "Donald", "Jared", "Robert", "Paul"];
