...
export class RepeatingWorldTimer {
   ...
   */
  public constructor(private readonly repeatPeriod: number, triggerNow: boolean = false) {
    if (!triggerNow)
      this.lastTriggeredTime = Time.gameTime();
    else
      this.lastTriggeredTime = Time.gameTime() - repeatPeriod;
  }

  public isReady(): boolean {
    return (
      Time.gameTime() >= this.lastTriggeredTime + this.repeatPeriod
    );
  }
